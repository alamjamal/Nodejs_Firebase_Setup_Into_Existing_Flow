...
const { getUserByEmail, verifyAccessToken, linkProviderToUser, revokeRefreshToken } = require('./user.public.service')
...
const registerNormalUser = async () => {
	try {
...
async function register(req, res) {
	...
	} else if (user?.password == "firebase") {
		user = await User.findByIdAndUpdate(user.id, { password: password }, { new: true })
	} else {
...
const loginFunction = async (res, user, loginWith, uid) => {
	const passCode = crypto.randomBytes(16).toString("hex");

	const accessToken = await generateAccessToken(user, loginWith, uid);
	const refreshToken = await generateRefreshToken(user, loginWith, uid);
...
async function login(req, res) {
	...
	if (user && bcrypt.compareSync(password, user.password)) {
		...
		loginFunction(res, user, "jwt", "uid")
	}
	else {
...
async function getAccessToken(req, res, next) {
	...
	const accessToken = await generateAccessToken(user, tokenDetails.loginWith, tokenDetails.uid);
	res.cookie('accessToken', accessToken, {
		httpOnly: true, //accessible only by web server 
		secure: true, //https
		sameSite: 'None',
...
async function logout(req, res) {
	...
	if (tokenDetails.loginWith !== "jwt") {
		await revokeRefreshToken(tokenDetails.uid)
	}
...
const firebaseLogin = async (req, res) => {
	let { accessToken, email, name, mobile } = req.body
	const decodedToken = await verifyAccessToken(accessToken);
	let findUser = await User.findOne({ email: email })
	// await findUser.remove({});

	if (decodedToken && findUser) {
		const firebaseIndex = findUser.firebase.findIndex(f => f.provider === decodedToken.firebase.sign_in_provider && f.uid === decodedToken.uid && f.picture === decodedToken.picture,);
		if (firebaseIndex == -1) {
			//not found
			findUser.firebase.push({
		...
		await findUser.save();
		loginFunction(res, findUser, decodedToken.firebase.sign_in_provider, decodedToken.uid)
	} else {
		let user = {
			email: email,
			name: name,
		...
		loginFunction(res, user, decodedToken.firebase.sign_in_provider, decodedToken.uid)
