...
const generateAccessToken = async (user, loginWith , uid) => {
	try {
		const payload = { id: user.id, roles: user.roles, email: user.email , loginWith:loginWith , uid:uid};
...
const generateRefreshToken = async (user,loginWith, uid) => {
	try {
		const payload = { id: user.id, roles: user.roles, email: user.email , loginWith:loginWith , uid:uid};
